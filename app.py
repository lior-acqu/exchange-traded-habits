    ...
    with get_db() as conn:
        with conn.cursor() as db:
            ...
            if not existing:
                ...
                db.execute('UPDATE habit_logs SET completed = 0, value = value * (1 - (0.01 / %s)) / 1.01 WHERE habit_id = %s AND date = %s', (float(interval), habit_id, today))
                db.execute('UPDATE habits SET current_value = current_value * (1 - (0.01 / %s)) / 1.01, change = (current_value / 1.01 * (100 - (1 / %s)) / initial_value) - 100 WHERE id = %s', (float(interval), float(interval), habit_id,))
                conn.commit()
            return redirect('/')
